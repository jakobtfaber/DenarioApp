"""
    ...
    Classes/Functions:
        - show_markdown_file(file_path: str, extra_format=False, label: str = '') -> None (line 36)
        - extract_api_keys(uploaded_file) (line 55)
        - set_api_keys(key_manager: KeyManager, api_key: str, llm: str) (line 73)
        - create_zip_in_memory(folder_path: str) (line 83)
        - get_project_dir() (line 99)
        - StreamToBuffer (line 110):
            - write(s) (line 115)
        - stream_to_streamlit(container) (line 122)
        - get_latest_mtime_in_folder(folder_path: str) -> float (line 136)
        - delete_old_folders(days_old: int = 1) (line 152)
...
try:
    ...
    # Fallback when imported as a script module (no package context)
    from constants import PROJECT_DIR, LLMs
...
def get_project_dir():
    ...
    # 1) If already set in session, reuse it
    if "project_dir" in st.session_state:
        return st.session_state.project_dir

    # 2) If env var provided, pin to that path across restarts
    env_dir = os.environ.get("DENARIO_PROJECT_DIR")
    if env_dir:
        os.makedirs(env_dir, exist_ok=True)
        st.session_state.project_dir = env_dir
        return st.session_state.project_dir

    # 3) Otherwise, create a new ephemeral per-session directory
